...
Uinsno = matrix_no_irrig_no_ins(1:fin,25:28)/10^7 % the third column of this matrix is max EU without ins
Uins= matrix_no_irrig_ins(1:fin,31:34)/10^7 % the 4th column is max EU with ins

% Getting the choice based on the proberence on expected utility
prob_uncertainty_no_irrig= zeros(fin,4)

for k= 1:4
for j=1:fin
prob_uncertainty_no_irrig(j,k) = exp(Uins(j,k))/(exp(Uins(j,k))+exp(Uinsno(j,k)))
end
end 

for k=1:4
prob_uncertainty_no_irrig(fin+1,k)= mean(prob_uncertainty_no_irrig(1:fin,k))
end 
