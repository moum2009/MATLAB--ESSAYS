...
% rainfall density 
R_W_g= @(W_g) exp(-(W_g-mu_W_g).^2/(2*sig_W_g.^2))*(1/(sig_W_g*(2*pi()).^.5)) % density function of precipitation 
...
EU_2= @(N,c) integral2(@(p,W_g) U_2(N,p,W_g,c).*R_W_g(W_g).*G_p(p),pmin,pmax,W_g_min,W_g_max)% Expected profit with no insurance 

EU_2_neg =@(x) - EU_2(x(1),x(2))
...
matrix_no_irrig_ins(j,18+k)= x_opt_2(1); matrix_no_irrig_ins(j,24+k)= x_opt_2(2); matrix_no_irrig_ins(j,30+k)= EU_2_opt;
   end
end

save matrix_no_irrig_ins.mat matrix_no_irrig_ins
